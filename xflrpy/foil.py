...
from xflrpy.polar import  PolarType, Analysis2dManager
...
class FoilManager(DictListInterface):
    ...
    def load(self, paths):
        ...
        for path in paths:
            if path[-4:].lower()!=".dat": 
                raise InvalidFoilPathError(f'Please provide a valid .dat file. "{path}" is invalid.')
        validation_result =  self._validate_file_paths(paths)
        for i,path in enumerate(paths):
            if validation_result[i] == False:
                raise InvalidFoilPathError(f'Please provide a valid file path. "{path}" is does not exist.')
        for path in paths:
    ...
    def create_naca_foil(self, digits, name=None):
        ...
        try:
            ...
            raise InvalidNacaValueError("ERROR - NACA foil value must be positive, 4 digit value")
